...
Heap<T>::Heap(T *arr, int size)
{
    ...
    copy(arr, arr + size, name_max_heap);
    copy(arr, arr + size, name_min_heap);
...
void Heap<T>::maxheapify(int i, const std::function<bool()> cmp)
{
    ...
    if (mx != i)
    {
        swap(name_max_heap[mx], name_max_heap[i]);
        maxheapify(mx);
    }
}
...
void Heap<T>::minheapify(int i, const std::function<bool()> cmp)
{
    ...
    if (mn != i)
    {
        swap(name_min_heap[mn], name_min_heap[i]);
        maxheapify(mn);
    }
}
...
void Heap<T>::print()
{
    for (int i = 0; i < size; ++i)
        cout << name_max_heap[i] << ' ';
    cout << '\n';
    for (int i = 0; i < size; ++i)
        cout << name_min_heap[i] << ' ';
}